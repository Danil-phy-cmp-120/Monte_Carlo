...
mcs_start = int(nmcs/10)
'''
ansver = input('To take into account the dependence of exchange integrals on temperature? (Yes/No):\n')
if ansver == 'Yes':
    J_T_accounting = True
else:
...
if J_T_accounting == True:
    ...
    Basis_ansver = input('Enter type of basis (sc/fcc/bcc):\n')

    if Basis_ansver == "sc": fact = 1.0
    if Basis_ansver == "bcc": fact = 0.5
    if Basis_ansver == "fcc": fact = 0.25

    IQ, conc = read_files.read_atoms(sorted(os.listdir(os.getcwd()))[0] + '/*JXC.out')

    murn, n = J_T.Murnaghan(fact, sum(conc))
    J = read_files.read_J(d_a, True, sorted(os.listdir(os.getcwd()))[n] + '/*JXC.out')
    J = J_T.J_T_accounting(T_start, T_end, T_step, fact, murn, sum(conc), J, d_a)
